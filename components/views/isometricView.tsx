...
export default function isometricView() {
    ...
    const buildingHeightPx = buildingHeight * scale;
    ...
    const isoMatrix = new DOMMatrixReadOnly()
        ...
        .skewX(-30)
        .scale(1, 0.8602)
        .translate(siteWidth * scale, -buildingDepthPx)


    const topMatrix = new DOMMatrixReadOnly()
        // .translate(20, 450)
        .translate(setbacks.left * scale, setbacks.back * scale)
        // .translate(building)
        ...
        <div className="isometric-view view">
            ...
            <svg className="graph" width={frontW + sideD} height={frontH * 2 + sideH}>
                ...
                <g transform={gridMatrix}>
                    <rect x={0} y={0} width={lotWidthPx} height={lotDepthPx} fill="url(#grid)" pointerEvents="none" />
                    <rect className="outer-stroke" x={0} y={0} width={lotWidthPx} height={lotDepthPx} />
                ...
                <g transform={frontMatrix}>
                    {Array.from({ length: visibleFloors }).map((_, i) => {
                        ...
                        const y = ((i) * fh);
                        return <rect className="building front" key={i} x={0} y={y} width={buildingWidthPx} height={fh} />;
                    })}
                ...
                <g transform={sideMatrix.toString()}>
                    {Array.from({ length: visibleFloors }).map((_, i) => {
                        ...
                        return <rect className="building side" key={i} x={y} y={0} width={fh} height={buildingDepthPx} />;
                    })}
                ...
                <g transform={topMatrix}>
                    <rect className="building top" x={0} y={0} width={buildingWidthPx} height={buildingDepthPx} />
